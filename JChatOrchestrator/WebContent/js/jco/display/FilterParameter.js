define(["jco/display/DatetimePicker","jco/utils/childtable",
    ...
    "dijit/form/FilteringSelect","dojo/store/Memory","dijit/registry"],
    function (displayDatetimePicker,childtable,
    		domconstruct,textbox,datetextbox,timetextbox,filteringselect,memory,registry) {

var FilterParameter = function (parent,filter_fields) {
	 	var paramtable = childtable(parent,parent.id +'-table',1,3);
	  	var fieldselect = document.createElement('div');
	  	fieldselect.id = parent.id + 'fieldselect';
	 	paramtable.rows[0].cells[0].appendChild(fieldselect);
	  	var operatorselect = document.createElement('div');
	  	operatorselect.id = parent.id + 'operatorselect';
	  	operatorselect.style.width = '15px';
	 	paramtable.rows[0].cells[1].appendChild(operatorselect);
	  	var valuediv = document.createElement('div');
	  	valuediv.id = parent.id + 'valuediv';
	 	paramtable.rows[0].cells[2].appendChild(valuediv);
		...
		var operatorStore = new memory({data:[],idproperty:"name"});
	 	...
	 	if (registry.byId(fieldselect.id)) {
	 		 registry.byId(fieldselect.id).destroyRecursive();
	 	}
	 	var fieldFilterSelect = new filteringselect({
	        ...
	        operatorStore.setData(opsA);
		 	...
		 	if (registry.byId(operatorselect.id)) {
		 		 registry.byId(operatorselect.id).destroyRecursive();
		 	}
